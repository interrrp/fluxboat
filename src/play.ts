...
export class PlayCommand {
  ...
  public async run(): Promise<void> {
    ...
    if (request === null) {
      await this.message.reply("Usage: b!play <url or search query>");
      return;
    }

    if ("searchQuery" in request) {
      await this.message.reply("Search queries are unimplemented");
      return;
    }

    const guildId = this.message.guild?.id;
    if (guildId === undefined) {
      await this.message.reply("This command is server-only");
      return;
    }

    const voiceChannelId = this.voiceManager.getVoiceChannelId(guildId, this.message.author.id);
    if (voiceChannelId === null) {
      await this.message.reply("Join a voice channel first");
      return;
    }
